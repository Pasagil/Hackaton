...
void Map::add_room(const Room &room){
    for (int i = 0 ; i< room.get_size().first ; i++){
        for (int j = 0 ; j < room.get_size().second ; j++){
            ...
            if (i == room.get_size().first) (set(i+1+ room.get_place().first,j+room.get_place().second,'-'));
            if (j == 0) (set(i+ room.get_place().first, j-1+room.get_place().second, '|'));
            if (j == room.get_size().second) (set(i+ room.get_place().first,j+1+room.get_place().second,'|'));
